function [rotationAngle,X,Y,finalImage,errors,finalOriginalImage] = ...
            alignTwoImages(image1,image2,angleGuess,spacing,fractionalPixelAccuracy,noRotation,originalImage)
...
if ~noRotation
    ...
    % figure
    % rr = max(peaks) - min(peaks);
    % minr = min(peaks);
    % ptVals = (peaks-minr)./rr;
    % plot(thetas,(peaks-minr)./rr,'b.','linewidth',2)
    % hold on
    % if (peakIndex~=1) && (peakIndex ~= N)
    %     plot([rotationAngle,rotationAngle],[0,(p(1)*rotationAngle^2+p(2)*rotationAngle+p(3)-minr)./rr],'r--','linewidth',2)
    %     plot(rotationAngle,(p(1)*rotationAngle^2+p(2)*rotationAngle+p(3)-minr)./rr,'cs','MarkerFaceColor','c')
    % else
    %     plot([rotationAngle,rotationAngle],[0,(peaks(peakIndex)-minr)/rr],'r--','linewidth',2);
    %     plot(rotationAngle,(peaks(peakIndex)-minr)/rr,'cs','MarkerFaceColor','c')
    % end
