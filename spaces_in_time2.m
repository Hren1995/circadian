function [sp,sub_beginds,sub_files] = spaces_in_time2(idx,pos_dir,em_dir)
	...
	tot_len = 0;
    sub_beginds = zeros(numel(g),1);
    sub_files = cell(size(g));
	for i=1:numel(g) % g should come out in sorted order, which is the same as chronological order
        sub_files{i} = embedding_to_movie(g{i});
        sub_beginds(i) = tot_len+1;
